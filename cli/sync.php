...
/**
 * CLI sync for full cohort synchronisation.
 *
...
cli_problem('[LOCAL COHORTSYNC] The sync enrolments cron script has been deprecated. Please use the scheduled task instead.');

// Abort execution of the CLI script if the enrol_ldap\task\sync_enrolments is enabled.
$task = \core\task\manager::get_scheduled_task('local_cohortsync\task\sync_cohorts');
if (!$task->get_disabled()) {
    cli_error('[LOCAL COHORTSYNC] The scheduled task sync_enrolments is enabled, the cron execution has been aborted.');
}
$starttime = microtime();

$plugin = new \local_cohortsync();
